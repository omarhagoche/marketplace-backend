    ...
    {!! Form::open(['route' => ['orders.store-order-restaurant-coupon',$order->id], 'method' => 'post']) !!}
     ...
     @else
        ...
        <div class="form-group row ">
            <div class="checkbox icheck col-9">
                <label class="form-check-inline">
                    {!! Form::open(['route' => ['operations.order.remove.restaurant.coupon', $order->restaurantCoupon->id], 'method' => 'delete']) !!}
                    {!! Form::button('Delete <i class="fa fa-trash"></i>', [
                        'type' => 'submit',
...
<div style="flex: 50%;max-width: 50%;padding:17px; border-style: solid; border-width: 1px; border-radius: 9px; border-color: #b5b5b538;" class="column">
    ...
    {!! Form::close() !!}
     ...
     <div class="form-group row ">
         <div class="checkbox icheck col-9">
             <label class="form-check-inline">
                 {!! Form::open(['route' => ['operations.order.remove.delivery.coupon', $order->deliveryCoupon->id], 'method' => 'delete']) !!}
                 {!! Form::button('Delete <i class="fa fa-trash"></i>', [
                     'type' => 'submit',
                     'class' => 'btn btn-link text-danger',
                     'onclick' => "return confirm('Are you sure?')"
